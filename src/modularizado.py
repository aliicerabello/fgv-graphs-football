...
opcao = input("Usar ID aleatório? (s/n): ")

if opcao.lower() == 's':
    try:
        MATCH_ID = selecionar_jogo_aleatorio()
        if MATCH_ID:
            print(f"ID aleatório: {MATCH_ID}")
        else:
...
try:
    ...
    for time in [time1, time2]:
        ...
        print(
            f"    {G.number_of_nodes()} jogadores, {G.number_of_edges()} conexões")

        # Acha jogador mais conectado
        graus = dict(G.degree(weight='weight'))
        top_jogador = max(graus.items(), key=lambda x: x[1])
        print(
            f"    Mais conectado: {top_jogador[0]} ({top_jogador[1]} conexões)")
