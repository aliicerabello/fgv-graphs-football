...
\begin{document}
	...
	\begin{table}[H]
		\centering
		\caption{Graus de Entrada e Saída - Argentina (Top 3)}
		\label{tab:graus_arg}
		\begin{tabular}{lcccc}
			\toprule
	...
	\begin{table}[H]
		\centering
		\caption{Graus de Entrada e Saída - França (Top 3)}
		\label{tab:graus_fra}
		\begin{tabular}{lcccc}
			\toprule
			\textbf{Jogador} & \textbf{Grau Saída} & \textbf{Grau Entrada} & \textbf{Dif.} & \textbf{Papel Tático} \\
	...
	\begin{table}[H]
		\centering
		\caption{Matchings Mais Relevantes - Argentina vs França (Top 3)}
		\label{tab:matchings_arg_fra}
		\begin{tabular}{lclc}
			\toprule
			\textbf{Atacante} & \textbf{Defensor} & \textbf{Peso} & \textbf{Significado Tático} \\
			\midrule
			Lionel Messi & Eduardo Camavinga & 6 & Marcação específica ao criador \\
			Ángel Di María & Jules Koundé & 3 & Duelo técnico na lateral direita \\
			Gonzalo Montiel & Eduardo Camavinga & 3 & Batalha pelo flanco direito \\
			\bottomrule
		\end{tabular}
	\end{table}
	
	Os matchings revelam que a Argentina focou em explorar os flancos, com Di María buscando constantemente o duelo contra Koundé. O matching Messi-Camavinga (peso 6) foi o mais intenso, mostrando a estratégia francesa de designar Camavinga para neutralizar o principal criador argentino.
	
	\begin{table}[H]
		\centering
		\caption{Matchings Mais Relevantes - França vs Argentina (Top 3)}
		\label{tab:matchings_fra_arg}
		\begin{tabular}{lclc}
			\toprule
			\textbf{Atacante} & \textbf{Defensor} & \textbf{Peso} & \textbf{Significado Tático} \\
			\midrule
			Eduardo Camavinga & Leandro Paredes & 3 & Contenção do organizador argentino \\
			Randal Kolo Muani & Enzo Fernandez & 3 & Pressão sobre o meia central \\
			Kylian Mbappé & Nicolás Otamendi & 2 & Duelo de velocidade no ataque \\
			\bottomrule
		\end{tabular}
	\end{table}
	
	Do lado francês, os matchings mostram uma estratégia de pressão distribuída no meio-campo. Camavinga contra Paredes (peso 3) revela o foco em neutralizar o organizador de jogo argentino, enquanto Kolo Muani pressionava Enzo Fernandez (peso 3) para dificultar a saída de bola. Mbappé contra Otamendi (peso 2) representou o clássico duelo entre velocidade e experiência na defesa.
	...
	\begin{table}[H]
		\centering
		\caption{Graus de Entrada e Saída - Espanha (Top 3)}
		\label{tab:graus_esp}
		\begin{tabular}{lcccc}
			\toprule
			\textbf{Jogador} & \textbf{Grau Saída} & \textbf{Grau Entrada} & \textbf{Dif.} & \textbf{Papel Tático} \\
	...
	\begin{table}[H]
		\centering
		\caption{Graus de Entrada e Saída - Marrocos (Top 3)}
		\label{tab:graus_mar}
		\begin{tabular}{lcccc}
			\toprule
			\textbf{Jogador} & \textbf{Grau Saída} & \textbf{Grau Entrada} & \textbf{Dif.} & \textbf{Papel Tático} \\
	...
	\begin{table}[H]
		\centering
		\caption{Matchings Mais Relevantes - Espanha vs Marrocos (Top 3)}
		\label{tab:matchings_esp_mar}
		\begin{tabular}{lclc}
			\toprule
			\textbf{Atacante} & \textbf{Defensor} & \textbf{Peso} & \textbf{Significado Tático} \\
			\midrule
			Ferran Torres & Noussair Mazraoui & 6 & Duelo de velocidade na lateral \\
			Pedri & Ounahi & 3 & Batalha pelo meio-campo \\
			Rodrigo Hernández & Sofiane Boufal & 2 & Contenção do criador marroquino \\
			\bottomrule
		\end{tabular}
	\end{table}
	
	O matching mais relevante foi Ferran Torres contra Mazraoui (peso 6), mostrando que a Espanha focou seu ataque pela direita contra o lateral marroquino.
	
	\begin{table}[H]
		\centering
		\caption{Matchings Mais Relevantes - Marrocos vs Espanha (Top 3)}
		\label{tab:matchings_mar_esp}
		\begin{tabular}{lclc}
			\toprule
			\textbf{Atacante} & \textbf{Defensor} & \textbf{Peso} & \textbf{Significado Tático} \\
			\midrule
			Sofiane Boufal & Sergio Busquets & 3 & Pressão sobre o pivô espanhol \\
			Achraf Hakimi & Daniel Carvajal & 2 & Duelo de laterais ofensivos \\
			Achraf Hakimi & Rodrigo Cascante & 2 & Exploração do corredor direito \\
			\bottomrule
		\end{tabular}
	\end{table}
	
	Os matchings marroquinos revelam uma estratégia bem definida: Boufal pressionando Busquets (peso 3) para cortar a saída de bola espanhola, enquanto Hakimi explorava tanto Carvajal quanto Cascante (peso 2 cada) no flanco direito, buscando criar superioridade numérica nesse setor.
	...
	Em contraste, a eficiência marroquina valida as observações táticas sobre a organização defensiva compacta. O matching Ferrán-Mazraoui (peso 6) ilustra perfeitamente como o Marrocos venceu a batalha tática: Enquanto Ounahi combatia no meio do campo, Mazraoui negou consistentemente o espaço no flanco esquerdo, forçando a Espanha a circular a bola em zonas inofensivas, conforme detalhado no relatório técnico da partida \cite{fifa_morocco_spain_report}.
	
	\section{Limitações Metodológicas}
	
	Embora a metodologia desenvolvida tenha se mostrado robusta e informativa, várias limitações importantes devem ser reconhecidas. A precisão das análises está intrinsecamente vinculada à acurácia e completude dos dados fornecidos pelo StatsBomb, onde eventuais inconsistências na categorização de eventos podem distorcer significativamente os resultados. Além disso, fatores contextuais cruciais como o resultado momentâneo, o minuto da partida e condições climáticas não são incorporados no modelo, limitando a interpretação situacional dos padrões identificados. A análise de redes estáticas não captura a evolução tática durante a partida ou substituições estratégicas em tempo real, representando uma visão agregada que pode mascarar ajustes táticos importantes. Por fim, os critérios estabelecidos para passes decisivos envolvem escolhas arbitrárias, como a janela temporal de 5 segundos para passes que geram chutes, que podem influenciar significativamente os resultados obtidos.
	
	\vspace{0.5cm}
	\noindent \textbf{Perspectivas Futuras}
	
	As limitações identificadas sugerem direções promissoras para pesquisas futuras, incluindo o desenvolvimento de redes temporais dinâmicas que capturem a evolução das redes de passes ao longo da partida, a utilização dos padrões identificados para desenvolver modelos preditivos de eficácia ofensiva, e a expansão da metodologia para capturar padrões defensivos coletivos através de métricas de pressão coordenada.
	...
	\begin{thebibliography}{99}
		...
		\bibitem{aronna2025}
		Aronna, M. S.
		\textit{Notas de Aula: Matemática Discreta}.
		FGV, 2025.
